...
A `copy` dependency, however, requires a `~Escapable` parameter type:
...
If the specialized type of the copied dependency's source is Escapable, then the dependency target is unconstrained by the source. Safety dictates, however, that a non-Escapable result requires a lifetime constraint in the caller. This leads to the following *requires-escapable" rule*:

If the sources of a `copy` dependency are all conditionally Escapable, then the target type must be Escapable whenever the source's specialized types are all Escapable.
...
The lifetime annotation (`copy a, copy b`) is valid if-and-only-if: `A: Escapable & B: Escapable` requires `R: Escapable`. This holds for any number of conditionally escapable sources.

With the requires-escapable rule, the compiler knows statically whether `R` can depend on `A` and `B` inside a generic context. The following examples show how theis rule affects diagnostics.

### Requires-escapable rule examples

1. ERROR: unrelated type parameters
```swift
@_lifetime(copy t) // ERROR: 'T' may be non-Escapable when 'U' is Escapable
func foo<T: ~Escapable, U: ~Escapable>(t: T) -> U
```

The source is conditionally Escapable on a type parameter unrelated to the target's Escapable condition.
...
Identical types are trivially Escapable under the same condition.

3. OK: nominal type with same type parameter
```swift
...
The source is conditionally Escapable on the same type parameter as the target.
...
5. ERROR: conditionally Escapable on insufficient type parameters

```swift
@_lifetime(copy t) // ERROR: 'T' may be non-Escapable when 'U' is Escapable
func foo<T: ~Escapable, U: ~Escapable>(t: T) -> (T, U)
```

The result is conditionally Escapable on multiple type parameters `U` and `T`, but the dependency source is only conditionally Escapable on `T`.

6. OK: conditionally Escapable on all type parameters

```swift
@_lifetime(copy t, copy u) // OK
func foo<T: ~Escapable, U: ~Escapable>(t: T) -> (T, U)
```

The result is conditionally Escapable on multiple type parameters and depends on multiple sources that are together conditionally Escapable on all those type parameters.

7. OK: conditionally Escapable dependency on an unconditionally Escapable source

```swift
struct NE3<T>: ~Escapable {...}

@_lifetime(copy ne) // OK
func foo<T, U: ~Escapable>(ne: NE3<T>) -> U
```

The source type, `NE3<T>`, is unconditionally non-Escapable because it has no extension that would create an Escapable conformance (Escapable conformance need to be declared in the same source file).

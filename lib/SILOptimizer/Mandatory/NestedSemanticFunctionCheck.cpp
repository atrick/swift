...
class NestedSemanticFunctionCheck : public SILFunctionTransform {
  // Mark all functions that may indirectly call a semantic function but are not
  // themselves an optimizable semantic function or semantic call wrapper.
  //
...
void NestedSemanticFunctionCheck::run() {
  ...
  // Add this function to mayCallSemanticFunctions or semanticCallWrappers if
  // needed.
  ...
  for (auto &bb : *f) {
    for (auto &i : bb) {
      ...
      // If this is a trivial wrapper around a semantic call, ignore it.

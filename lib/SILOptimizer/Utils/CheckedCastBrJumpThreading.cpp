...
/*!!!
CopyValueInst *CheckedCastBranchInst::Edit::copySuccessArg() {
  SILBuilderWithScope builder(SuccessArg->getParent());
  auto loc = RegularLocation::getAutoGeneratedLocation(
    SuccessArg->getLoc().getSourceLoc());
  return builder.createCopyValue(loc, SuccessArg);
}
*/
  ...
  if (hasUnknownPreds) {
    if (!SuccessPreds.empty()) {
      ...
      if (!CCBBlock->getParent()->hasOwnership()) {
        ...
        Builder.createBranch(clonedCCBI->getLoc(), successBB);
        assert(successBB->getNumArguments() == 1);
        //!!! use RAUW helper
        successBB->getArgument(0)->replaceAllUsesWith(SuccessArg);
      ...
      for (auto *Pred : SuccessPreds) {
        ...
        replaceBranchTarget(TI, CCBBlock, TargetSuccessBB,
                            /*PreserveArgs=*/true);
      }
    }
    return;

...
bool InstructionDeleter::
replaceOperandAndDeleteIfDead(Operand *operand, SILValue newValue) {
  SILValue oldValue = operand->get();
  if (newValue == oldValue)
    return false;

  SILFunction *function = operand->getUser()->getFunction();
  bool fixLifetimes = function && function->hasOwnership();

  auto *oldDef = oldValue->getDefiningInstruction();
  auto cleanupOldDef = [&]() {
    if (oldDef && isDead(oldDef)) {
      deleteWithUsesAndCleanup(oldDef, fixLifetimes);
      return true;
    }
    return false;
  };
  ...
  if (!fixLifetimes) {
    ...
    return cleanupOldDef();
  }
  if (Operand *singleUse = getSingleLiveUse(oldDef)) {
    assert(singleUse == operand);
    operand->set(newValue);
    bool deleted = cleanupOldDef();
    assert(deleted);
    return deleted;
  }
  if (auto *newInst = replaceOSSAOperand(operand, newValue)) {
    getCallbacks().createdNewInst(newInst);
  }
  return cleanupOldDef();
}

bool InstructionDeleter::
replaceAllUsesAndDeleteIfDead(SILValue oldValue, SILValue newValue) {
  getCallbacks().notifyWillReplaceUses(oldValue, newValue);

  oldValue->replaceAllUsesWith(newValue);

  auto *oldDef = oldValue->getDefiningInstruction();
  if (oldDef && isDead(oldDef)) {
    bool fixLifetimes = oldDef->getFunction()->hasOwnership();
    deleteWithUsesAndCleanup(oldDef, fixLifetimes);
    return true;
  }
  // If oldValue cannot be deleted, complete its ownership.
  if (oldValue.getOwnershipKind() == OwnershipKind::Owned) {
    auto loc = RegularLocation::getAutoGeneratedLocation();
    // insertPt is non-null because Undef is non-owned.
    auto *insertPt = oldValue->getNextInstruction();
    SILBuilderWithScope(insertPt).createDestroyValue(loc, oldValue);
  }
  return false;
}

bool InstructionDeleter::isDead(SILInstruction *inst) {
  bool fixLifetime = inst->getFunction()->hasOwnership();
  return (isInstructionTriviallyDead(inst)
...
void InstructionDeleter::deleteWithUses(
    SILInstruction *inst, bool fixLifetimes, bool forceDeleteUsers,
    SmallVectorImpl<SILInstruction *> *deadOperands) {
  ...
  for (auto *inst : toDeleteInsts) {
    for (Operand &operand : inst->getAllOperands()) {
      ...
      if (deadOperands && operDef) {
        if (Operand *singleUse = getSingleLiveUse(operDef)) {
          assert(singleUse->getUser() == inst);
          operand.drop();
          deadOperands->push_back(operDef);
          continue;
        }
      }
      if (fixLifetimes) {
        dropOSSAOperand(&operand);
      } else {
        operand.drop();
      }
      if (operDef) {
...
void InstructionDeleter::deleteWithUsesAndCleanup(SILInstruction *inst,
                                                  bool fixLifetimes,
                                                  bool forceDeleteUsers) {
  SmallVector<SILInstruction *, 4> deadOperands;
  deleteWithUses(inst, fixLifetimes, forceDeleteUsers, &deadOperands);
  while (!deadOperands.empty()) {
    SILInstruction *deadInst = deadOperands.pop_back_val();
    // The single-use check in deleteWithUses should prevent redundant
    // deadOperands, but check to be safe.
    if (deadInst->isDeleted())
      continue;

    assert(isDead(deadInst));
    deleteWithUses(deadInst, fixLifetimes, forceDeleteUsers, &deadOperands);
  }
}

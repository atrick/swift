...
/// avoided with trivial dominance check.
struct BorrowedLifetimeExtender {
  ...
  void discoverReborrow(PhiValue reborrowedPhi) {
    // initially, map the reborrowed phi to an invalid owned phi
...
SILValue BorrowedLifetimeExtender::createCopyAtEdge(PhiOperand reborrowOper) {
  ...
  return SILBuilderWithScope(branch).createCopyValue(loc,
                                                     reborrowOper.getSource());
}
...
void BorrowedLifetimeExtender::extendOverBorrowScope(SILValue ownedValue) {
  ...
  for (PhiValue reborrowedPhi : reborrowedPhis) {
    ...
    for (auto *predBlock : phiBlock->getPredecessorBlocks()) {
      ...
      addNewEdgeValueToBranch(ti, phiBlock, undef, callbacks);
    }
...
struct OwnershipRAUWUtility {
  ...
  SILInstruction *getConsumingPoint() const {
    if (auto *blockArg = dyn_cast<SILPhiArgument>(oldValue))
      return blockArg->getTerminatorForResultArg();
...
class GuaranteedPhiBorrowFixup {
  // Avoid processing the same phi twice.
  SmallPtrSet<SILPhiArgument *, 8> discoveredPhis;

  // A phi in mustConvertPhis has already been determined to be part of this
  // new nested borrow scope.
  SmallVector<SILPhiArgument *, 8> mustConvertPhis;

public:
  // Determine if this new phi needs a nested borrow scope. If so, seed the
  // mustConvertPhis worklist.
  GuaranteedPhiBorrowFixup(SILPhiArgument *newPhi) {
    ...
    if (!newPhi->visitIncomingPhiOperands(
      ...
      discoveredPhis.insert(newPhi);
      mustConvertPhis.push_back(newPhi);
    }
  }

  /// Return true if an extended nested borrow scope was created.
  bool createExtendedNestedBorrowScope();
  ...
  for (SILInstruction *lastUser : boundary.lastUsers) {
    ...
    if (isa<BranchInst>(lastUser)) {
      for (Operand &oper : lastUser->getAllOperands()) {
        ...
        SILPhiArgument *phiVal = PhiOperand(&oper).getValue();
        if (discoveredPhis.insert(phiVal).second) {
          mustConvertPhis.push_back(phiVal);
        }
        continue;
      }
    }
...
bool GuaranteedPhiBorrowFixup::createExtendedNestedBorrowScope() {
  bool createdBorrowScope = false;
  while (!mustConvertPhis.empty()) {
    SILPhiArgument *phi = mustConvertPhis.pop_back_val();
    if (phi->visitIncomingPhiOperands(
            [&](Operand *op) { return !phiOperandNeedsBorrow(op); })) {
      // All operands were visited without needing a borrow.
      continue;
    }
    createdBorrowScope = true;
    // Borrow all operands that don't come from a discovered phi
    phi->visitIncomingPhiOperands([&](Operand *op) {
      // !!!FIXME!!! Handle recurion: first discover all phis before borrowing
      // !!!operands,
      borrowPhiOperand(op);
      return true;
    });
    // At least one operand needs a nested borrow. Therefore, create a new
    // borrow scope for all operands and uses.
    insertEndBorrowsAndFindPhis(phi);
  }
  return createdBorrowScope;
}
...
bool swift::createBorrowScopeForPhiOperands(SILPhiArgument *newPhi) {
  return GuaranteedPhiBorrowFixup(newPhi).createExtendedNestedBorrowScope();
}

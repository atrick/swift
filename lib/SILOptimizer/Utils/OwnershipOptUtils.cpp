...
OwnershipLifetimeExtender::borrowCopyOverScope(SILValue newValue,
  ...
  ctx.getCallbacks().createdNewInst(copy);
  ...
  BorrowedLifetimeExtender(borrowedValue, ctx.getCallbacks())
      .extendOverBorrowScopeAndConsume(copy);

  auto *borrow = builder.createBeginBorrow(loc, copy);
  ctx.getCallbacks().createdNewInst(borrow);
  return borrow;
}
  ...
  // Create end_borrows at the end of the borrow's lifetime.
  {
    ...
    borrowBoundary.visitInsertionPoints(
        [&](SILBasicBlock::iterator insertPt) {
          ...
          ctx.getCallbacks().createdNewInst(endBorrow);
        },
        &ctx.deBlocks);
  }
  ...
  // uses that end the borrow scope.
  {
    ...
    copyBoundary.visitInsertionPoints(
        [&](SILBasicBlock::iterator insertPt) {
          ...
          ctx.getCallbacks().createdNewInst(destroy);
        },
        &ctx.deBlocks);
  }
  return borrow;
...
OwnershipLifetimeExtender::borrowOverSingleUse(SILValue newValue,
  ...
  ctx.getCallbacks().createdNewInst(endOldBorrow);
  ...
  oldBorrowedVal.visitLocalScopeEndingUses([&](Operand *endScope) {
    ...
    ctx.getCallbacks().createdNewInst(endNewBorrow);
    return true;
  });
...
SILValue OwnershipRAUWPrepare::prepareUnowned(SILValue newValue) {
  auto newOwnership = newValue.getOwnershipKind();
  if (newOwnership == OwnershipKind::Unowned) {
    // An unowned value can always be RAUWed with another unowned value.
    return newValue;
  }
  // TODO: Check if the owned or guaranteed value is already available across
  // all uses. If so, simply return the newValue with no preparation.
  if (newOwnership == OwnershipKind::Guaranteed
      && isa<SILFunctionArgument>(newValue)) {
    return newValue;
  }
  // Insert a conversion instruction before an unowned return so that the end
  // of the borrowed-copy scope has a valid insertion point.
  //
  // This loop modifies the use list.
  auto useIter = oldValue->use_begin();
  while (useIter != oldValue->use_end()) {
    Operand *use = *(useIter++);
  ...
  assert(newOwnership == OwnershipKind::Owned);
  // Insert a copy of the owned value and lifetime extend that over
...
SILValue OwnershipRAUWPrepare::prepareReplacement(SILValue newValue) {
  ...
  case OwnershipKind::Owned: {
    ...
    return extender.createPlusOneCopy(newValue, consumingPoint);
  }
...
OwnershipRAUWHelper::getReplacementAddress() {
  ...
  ctx->getCallbacks().createdNewInst(newBase);

...
void SILCombiner::canonicalizeOSSALifetimes(SILInstruction *currentInst) {
  ...
  CanonicalizeOSSALifetime canonicalizer(
      ...
      localDeleter);
  CanonicalizeBorrowScope borrowCanonicalizer(localDeleter);

  while (!defsToCanonicalize.empty()) {
    SILValue def = defsToCanonicalize.pop_back_val();

    localDeleter.getCallbacks().resetHadCallbackInvocation();

    auto canonicalized = [&]() {
      if (!localDeleter.getCallbacks().hadCallbackInvocation())
        return;

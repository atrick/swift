...
// CHECK:   destroy_addr [[A1:%.*]] : $*Any, loc {{.*}}:22:19, scope 3
// CHECK:   dealloc_stack %{{.*}} : $*Optional<Any>, loc {{.*}}:22:23, scope 3
// CHECK:   dealloc_stack [[A1]] : $*Any, loc {{.*}}:22:23, scope 3
// CHECK:   dealloc_stack %{{.*}} : $*A, loc {{.*}}:22:7, scope 3
